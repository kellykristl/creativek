...
try {
	...
	$message = filter_input(INPUT_POST, "contact-message", FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
	...
	$swiftMessage->setFrom([$email => $contact-name]);
	 ...
	 **/
	$swiftMessage->setBody($contact-message, "text/html");
	$swiftMessage->addPart(html_entity_decode($contact-message), "text/plain");
